...
function Out-Mission {
	...
	$ConfigTemplate = @'
{[String]Variable*:phx_gameMode} = {[String]Value:uplink}; //Change this to whatever game mode you want and configure the
respective file in mode_config
//If you don't want to use one of the template modes, you can set phx_gameMode = "";
{[String]Variable*:phx_defendingSide}= {[String]Value:east}; //west/east/independent
{[String]Variable*:phx_defendingSide}= {[String]Value:west}; //west/east/independent
{[String]Variable*:phx_defendingSide}= {[String]Value:independent}; //west/east/independent
{[String]Variable*:phx_attackingSide} = {[String]Value:west}; //west/east/independent
{[String]Variable*:phx_bluforUniform} = "{[String]Value:UNIFORM_MARPAT_WD}";
{[String]Variable*:phx_opforUniform}= "{[String]Value:UNIFORM_EMR_SUMMER}";
{[String]Variable*:phx_indforUniform} = "{[String]Value:UNIFORM_M93_OAKLEAF}";
{[String]Variable*:phx_civUniform} = "{[String]Value:UNIFORM_CIVILIAN}";
{[String]Variable*:phx_bluforWeapons}= "{[String]Value:WEAPONS_M4A1_BLOCK_M249_M240G_M136}";
{[String]Variable*:phx_opforWeapons} = "{[String]Value:WEAPONS_AK74M_PKM_PKP_RPG7}";
{[String]Variable*:phx_indforWeapons} = "{[String]Value:WEAPONS_AKMN_PKM_PKP_RPG7}";
{[String]Variable*:phx_civWeapons} = "{[String]Value:WEAPONS_UNARMED}";
{[String]Variable*:phx_bluAT}= {[String]Value:CARLG}; //BLUFOR
{[String]Variable*:phx_redAT} ={[String]Value:RPG32}; //OPFOR
{[String]Variable*:phx_grnAT} = {[String]Value:RPG32}; //INDFOR
{[String]Variable*:phx_allowFortify} = {[String]Value:true}; //Allow the defending side to use ACEX Fortify
{[String]Variable*:phx_fortifyPoints} = {[String]Value:90}; //Currency for fortify - individual to each Combat Engineer
{[String]Variable*:phx_enemyStartVisible} = {[String]Value:false}; //Set to false to hide enemy start zone markers
{[String]Variable*:phx_maxViewDistance} = {[String]Value:1000}; //Maximum client view distance (meters)
'@
	...
	# Mission Name
	$MissionNamePattern = '^FNF_[A-z0-9]+_[A-z0-9\-]+_[A-z0-9]+_v[0-9]{1,2}_(EU|NA|ANY)\.[A-Za-z0-9_]+$'
	...
	[System.Collections.ArrayList] $Weather = @()
	...
	# Lightning Start
	# startLightnings
	[void] $Weather.Add((Get-WeatherSetting 'Lightning Start' 'startLightnings[\s]*='))
	# Lightning Forecast
	# forecastLightnings
...
<html>
	...
	<body>
		...
		<div class="centercontent">
			...
			$($Weather | ConvertTo-Html -Fragment)
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUMpsjxS1/aAYv+rKOcVfYtVQi
# fHaggg1QMIIDFTCCAf2gAwIBAgIQarNwqALG/45HOA0xJfnsbTANBgkqhkiG9w0B
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUaKeQK0w8BtViKjKWaGNc5wlh
# 7KwwDQYJKoZIhvcNAQEBBQAEggEAFkDUHvY8+U1EbobQu5hP84EkIDnJ+7C5A2V6
# tmFLDlZJM2USVrPXRJHJBajhTqYNETdhs2wITraetSVHLSjLzgvMEntza/bcfHR4
# HbfHxlV4vy3JdSYAKvT7B+4hRwgfZN8wlIpT6MuIAIl1w59/XCT1JdRT3cnIDqab
# ki+Y2j+ktxVSsw1UN9dyRTxJ9mCxUHof2QVQxmJ829htfTwdxpDHACPBkgaoW4zN
# g0zDR2917Z0lFEQ/jWYa8/mH9w3LNFSIBJdkaNFe5r6LTaNyDXGweV1C+gtP5nH+
# UNs9mdIdPd3vXWuPFMkLqWp4hYsVe3+U2UtQDmuGr93cLtSco6GCAjAwggIsBgkq
...
# SIb3DQEHATAcBgkqhkiG9w0BCQUxDxcNMjEwMzI0MTIzNzA0WjAvBgkqhkiG9w0B
# CQQxIgQgdEVpSHHaonl23Od+9dH/1MPZUssiQNKz+CY9G0P6OREwDQYJKoZIhvcN
# AQEBBQAEggEAtktgbWqlf7uGPcNbHt8PoJLVXeo7j/GCDKWhxdpSdMx8UTHd6QB5
# s8WV+lycj21A1ip5B1sZKLMz+FIk2MdO2L7LbqKPCtgca1DiIwlYd5iarWRfv1/k
# L1qXSfkAdsDcIGng1z2aNyuXWcM6pXxMg7ep+yKO2Swf697auCvJhVd1EHVfVTv+
# +Mbkn460KcdBf5PwhqGycS90w1j4XTz5jsNlT+f3VjxxzwJRFcpv4K7hMpcN4brB
# /IoMbTqbakdIlo2N8ALKicyMoWriGKBD3ZB8e0e5tfliT0WuBV+MXy4RSq6w5DOl
# G9SLOZgVcAoGra4+oykTlvpys99bAiRhIQ==
